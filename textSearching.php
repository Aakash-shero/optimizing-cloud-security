            ...
            <div class="container">
                ...
                <?php
                    if($_SERVER["REQUEST_METHOD"] == "POST")
                    {
                        ...
                        mysqli_select_db($connect ,"be") or die("<br><br>no such database found!!!<br><br>");
                        
                        //count total files to compute IDF
                        $query11 = mysqli_query($connect," SELECT * FROM fileindex ") or die(mysqli_error());
                        $total_documents = mysqli_num_rows($query11);
                        ...
                        $query4 = mysqli_query($connect,"select * from topkresults") or die(mysqli_error());
                        if(mysqli_num_rows($query4))
                        {
                            //table is not empty, delete data
                            $query44 = mysqli_query($connect,"truncate table topkresults");	
                        }
                        $query7 = mysqli_query($connect,"select * from topkencryptresults") or die(mysqli_error());
                        if(mysqli_num_rows($query7))
                        {
                            //table is not empty, delete data
                            $query77 = mysqli_query($connect,"truncate table topkencryptresults");	
                        }
                        ...
                        if(strlen($text) > 3)
                        {
                            ...
                            {
                                $query1 = mysqli_query($connect," SELECT * FROM modifiedindex WHERE keyword like '%".$key."%' OR keyword like '".$key."%' OR keyword like '%".$key."' ") or die(mysqli_error());
                                ...
                                if(mysqli_num_rows($query1))
                                {
                                    while($rows = mysqli_fetch_assoc($query1))
                                    {
                                        ...
                                        if($id)
                                        {
                                            $query2 = mysqli_query($connect," SELECT file_name FROM fileindex WHERE file_id = $id ") or die(mysqli_error());
                                            if(mysqli_num_rows($query2))
                                            {
                                                $execute2 = mysqli_fetch_array($query2);
                                                $name = $execute2['file_name'];
                                                
                                                //count the occurance of a particular keyword in index table for computing IDF
                            $query3 = mysqli_query($connect," SELECT COUNT(keyword) as total FROM modifiedindex WHERE keyword like '%".$key."%' OR keyword like '".$key."%' OR keyword like '%".$key."' ") or die(mysqli_error());
                                                $execute3 = mysqli_fetch_array($query3);
                                                $occurance = $execute3['total'];
                                                    
                                                //calculating IDF for top k files retrival
                                                $idf = log($total_documents / $occurance) * $term_freq;
                                                        
                $query4 = mysqli_query($connect,"insert into topkresults(file_id, file_name, idf) values(".$id.", '".$name."', ".$idf.")") or die("<br>Insert Query Failed inserting to topkresults ".mysqli_error());
                                            }
                                            else
                                            {
                            ...
                            $query4 = mysqli_query($connect,"select * from topkencryptresults") or die(mysqli_error());
                    ...
                    $query44 = "insert into topkencryptresults(file_id, file_name, idf) values (AES_ENCRYPT('".$id."', '".$aesKey."'), AES_ENCRYPT('".$name."', '".$aesKey."'), ".$idf.")";
                                        $result44 = mysqli_query($connect,$query44) or die("<br>Insert Query Failed inserting to topkencryptresults ".mysqli_error());
                                            }
                                            else
                                            {
                                                //file id is not present, skip current keyword and move to next
                            ...
                            $result5 = mysqli_query($connect,$query5) or die("<br>Query Failed selecting from topkencryptresults ".mysqli_error());
                            ...
                            $result55 = mysqli_query($connect,$query55) or die("<br>Query Failed selecting from topkresults ".mysqli_error());
                            $numrows55 = mysqli_num_rows($result55);			
                            if($numrows55 > 0)
                            {
                                echo "<table border='1' cellspacing='4' cellpadding='4' align='center'>";
                                ...
                                $previous_idf = 0;
                                $filename = array();
                                while($rows55 = mysqli_fetch_assoc($result55))
                                {
                                    $id = $rows55['file_id'];
