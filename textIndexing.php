				...
				error_reporting(E_ERROR | E_WARNING | E_PARSE);
					...
					{					
						...
						function uploadFile()
						{
							...
							$target_dir = "C:\\xampp\htdocs\Html/";
						...
						$handle = fopen("C:\\xampp\htdocs\Html/".$my_file, "r") or die('Cannot open file: '.$my_file);
							$filesize = filesize("C:\\xampp\htdocs\Html/".$my_file);
						...
						function getID($filename, $filesize)
						{
							...
							$connect = new mysqli("localhost","root","") or die(mysql_error());
							mysqli_select_db($connect,"be") or die("<br><br>no database found<br><br>");
							
							$query1 = mysqli_query($connect, " select * from fileindex where file_name = '$filename' ");
							...
							else 
							{
							  ...
							  $result = mysqli_query($connect,$query2) or die("<br>Query Failed inserting to fileindex ".mysqli_error());
							}
								
							$query3 = mysqli_query($connect," select file_id from fileindex where file_name = '$filename' ");
							$execute = mysqli_fetch_array($query3);
							$id = $execute['file_id'];
							...
							else 
							{
							  ...
							  $result22 = mysqli_query($connect,$query22) or die("<br>Query Failed inserting to encryptedfileindex ".mysqli_error());
							}
						...
						function toDatabase($file_id, $words, $count_words)
						{
							...
							$query1 = mysqli_query($connect," select * from modifiedindex where file_id = $file_id ");
							if(mysqli_num_rows($query1))
							{
							...
							else 
							{
							  ...
							  foreach ($words as $key => $val) 
							  {
									...
									$result = mysqli_query($connect,$query2) or die("<br>Query Failed inserting to modifiedindex ".mysqli_error());
									...
									$result22 = mysqli_query($connect,$query22) or die("<br>Insert Query Failed inserting to encryptedindex ".mysqli_error());		
							  }	
							}
						...
						function display($filename, $contents)
						{
							...
							$filesize = filesize("C:\\xampp\htdocs\Html/".$filename);
							$aesKey = "Mary has one cat";
						...
						$result11 = mysqli_query($connect,$query11) or die("<br>Query Failed selecting from encryptedfileindex ".mysql_error());
							...
							$result22 = mysqli_query($connect,$query22) or die("<br>Query Failed selecting from encryptedindex ".mysqli_error());
							$numrows22 = mysqli_num_rows($result22);			
							if($numrows22 > 0)
							{
								echo "<table border='1' cellspacing='4' cellpadding='4' align='center'>";
								echo "<tr><th>ID</th><th>Keyword</th><th>Frequency</th><th>Term_Frequency</th></tr>";
